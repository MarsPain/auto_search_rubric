...
本次审计对 85 个 Python 源文件、28 个测试文件、约 13,000 行生产代码进行了全面扫描。结论：**代码结构良好、分层清晰，但存在 6 项架构级风险、8 项代码级缺陷、7 项质量级缺口**。建议在进入 Stage E（Classifier RM 蒸馏）之前，集中一个 Sprint（约 2 周）完成"技术债务清偿"，否则上层模块将继承底层的不稳定性。

**关键数字：**

| 维度 | 数量 | 说明 |
|------|------|------|
| 🔴 架构级债务 | 6 | 可能在未来导致大规模重构 |
...
### 1.1 SearchSession 与 EvolutionaryRTDSearcher 紧耦合

| 属性 | 内容 |
|------|------|
| **位置** | `autosr/harness/session.py` 第 335、508、554、557、562、570、579、593、600、607、623、630 行 |
| **描述** | `SearchSession` 直接调用 `EvolutionaryRTDSearcher` 的 10+ 个私有方法（`_init_global_state`, `_score_population`, `_log_generation_progress`, `_update_generation_bests`, `_handle_stagnation`, `_select_hard_prompts`, `_evolve_selected_prompts`, `_finalize_best_from_population`, `_collect_margin_improvement` 等）。 |
...
| **建议修复** | 1. 在 `autosr/interfaces.py` 中定义 `SteppableSearcher` Protocol，暴露 `get_algorithm_state()` / `restore_algorithm_state()` / `step()` / `is_finished()` 等方法；<br>2. 将 `EvolutionaryRTDSearcher` 的私有方法中需要被 harness 调用的部分提升为协议实现；<br>3. `SearchSession` 仅通过协议与 searcher 交互。 |
| **估算** | 3–4 天（含测试改造） |
| **阻塞** | 是。不解决此问题，Stage E 中引入新的搜索变体（如 classifier-guided search）将极其困难。 |

---

### 1.2 缺少正式的 Steppable Searcher 协议

| 属性 | 内容 |
|------|------|
| **位置** | `autosr/interfaces.py`、`autosr/harness/session.py` |
| **描述** | 当前只有 `Verifier`、`PreferenceJudge`、`RubricProposer`、`RubricInitializer` 四个协议。搜索算法本身没有协议约束，`factory.py` 直接返回具体类。 |
| **影响** | 中高。导致 1.1 的紧耦合问题无法通过类型系统发现；也无法在编译期保障新搜索算法满足 harness 需求。 |
| **根因** | 早期设计将搜索视为单一 `run()` 调用，未预见 step-wise execution 和 checkpoint 会成为底座能力。 |
| **建议修复** | 与 1.1 同步实施。定义 `Searcher` + `SteppableSearcher` 两级协议。 |
| **估算** | 与 1.1 合并计算 |

---

### 1.3 Harness 的 step-wise 执行范围残缺
...
| **建议修复** | 1. 明确 "CLI 配置 → 算法配置" 的单向转换层（如 `EvolutionaryConfig.from_runtime(config.search)`）；<br>2. 算法层不再独立维护默认值，全部从 RuntimeConfig 推导；<br>3. 在 `__post_init__` 中做统一校验。 |
| **估算** | 1 天 |

---

### 1.6 `CheckpointCallback` 类型别名重复
...
| **描述** | 多处使用裸 `except Exception` 或 `except Exception as e` 捕获所有异常。在 checkpoint 保存失败时返回 `None` 或记录 warning 后继续，可能将磁盘满、权限错误、序列化 bug 等真实问题吞掉。 |
| **影响** | 中。在长线搜索任务中，checkpoint 静默失败意味着用户以为状态已保存，实际没有；恢复时会丢失大量进度。 |
| **根因** | 过度防御式编程，未区分"预期内可恢复错误"和"预期外系统错误"。 |
| **建议修复** | 1. 定义 `CheckpointIOError` 等细分异常；<br>2. 对 `OSError`（磁盘、权限）做显式处理并 loud fail（至少记录 ERROR 并抛出）；<br>3. 对序列化错误（`TypeError`、`ValueError`）做 loud fail，因为这类错误通常是代码 bug，不应该被静默吞掉。 |
| **估算** | 0.5 天 |
...
| **影响** | 低。属于接口污染——调用方传递了数据集，但工厂不使用，增加了认知负担。 |
| **根因** | 早期某版本曾根据 prompts 内容动态决定提取策略，后来移除但保留了参数。 |
| **建议修复** | 移除 `prompts` 参数，同步修改所有调用点（`factory.py` 内仅 2 处）。 |
| **估算** | 0.25 天 |
...
| **影响** | 中高。随着代码量增长（当前 85 个文件），人工 code review 无法覆盖所有类型不一致、未使用变量、导入循环、格式问题。Stage E 计划引入更多模块，工具链缺失将显著降低开发效率。 |
...
| 属性 | 内容 |
|------|------|
| **位置** | `docs/PLANS.md` 第 111 行、`docs/exec-plans/active/` 目录 |
| **描述** | 1. `PLANS.md` 中 Stage D 标注为"🚧 设计完成，待实现"，但 `ROADMAP.md` 和实际代码均已确认 Stage D 完成；<br>2. `stage-d3-comparative-view.md` 位于 `active/` 目录，但所有任务和验收标准均已打勾。 |
| **影响** | 低。对开发者造成认知混乱，新人可能误以为 Stage D 尚未实现。 |
| **建议修复** | 1. 更新 `PLANS.md` Stage D 状态为 ✅ 已完成；<br>2. 将 `stage-d3-comparative-view.md` 移动到 `exec-plans/completed/`。 |
| **估算** | 15 分钟 |

---

### 3.7 `llm_components/factory.py` 与 `models.py` 遗留兼容层未标记

| 属性 | 内容 |
|------|------|
| **位置** | `autosr/llm_components/factory.py`、`autosr/models.py` |
| **描述** | `llm_components/factory.py` 的 `create_llm_components` 函数 docstring 标注为 legacy，但没有使用 `@warnings.deprecated` 或显式弃用日志；`models.py` 只是 `data_models.py` 的 re-export shim，也没有弃用标记。 |
| **影响** | 低。新开发者可能误用 legacy 入口，导致代码分散在两条实例化路径上。 |
| **建议修复** | 1. 对 `create_llm_components` 添加 `warnings.warn("deprecated", DeprecationWarning, stacklevel=2)`；<br>2. 对 `models.py` 的每个 re-export 添加 `typing_extensions.deprecated` 注解（Python 3.11+ 可用 `@warnings.deprecated`）；<br>3. 在 `AGENTS.md` 和 `DESIGN.md` 中记录弃用时间表。 |
| **估算** | 0.5 天 |
...
| 天 | 任务 | 对应债务项 |
|----|------|-----------|
| 1 | 修复 `TrainingManifest.from_json` 重复声明 + `LineageView` 可变默认 + `registry.py` 双重 I/O + `create_verifier_with_extraction` 参数清理 | 2.1, 2.4, 2.5, 2.8 |
| 2 | 统一原子写 JSON 原语 + 修复 `CheckpointCallback` 重复 + 选择策略 `id()` → fingerprint | 1.4, 1.6, 2.7 |
| 3 | 消除 `rm/use_cases.py` 硬编码默认值 + 修复 `_config_to_dict` 哈希碰撞风险 + 细化 `except Exception` | 2.2, 2.3, 2.6 |
| 4–5 | 设计并实现 `SteppableSearcher` 协议，解耦 `SearchSession` 与 `EvolutionaryRTDSearcher` | 1.1, 1.2 |
...
| 10 | 补齐 `prompt_local` / `iterative` 的 step-wise 执行 + 文档归档 + 废弃标记 | 1.3, 3.6, 3.7 |

### 4.2 不可跳过的阻塞项

以下债务**必须在进入 Stage E 之前解决**，否则上层建筑将继承不稳定性：

- **1.1 SearchSession 紧耦合** — Stage E 可能引入新的搜索策略（如 classifier-guided mutation），没有协议层无法集成。
...
- 1.3 `prompt_local` step-wise（除非 Stage E 明确需要 prompt-local 搜索）
- 2.7 `id()` 去重（当前无实际 bug）
- 3.6 文档归档（纯文档维护）
...
1. **代码扫描**：`grep -r` 搜索 TODO/FIXME/HACK/XXX/deprecated/bare-except/hardcoded-secrets 等模式（零 false positive）。
2. **测试覆盖映射**：将 28 个测试文件与 85 个源文件逐一对照，识别零覆盖模块。

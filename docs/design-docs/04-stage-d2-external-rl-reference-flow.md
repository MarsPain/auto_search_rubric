# Reward Harness Stage D2: External RL Repo Reference Flow

> **版本**: v1.1 | **状态**: 已实现 | **最后更新**: 2026-05-01
>
> 本文档描述外部 RL 仓库（以 verl 为例）如何接入 Reward Harness RM Server 与实验注册表。
...
Phase 3: Finalize
  ...
  -> record both into reward_harness registry
```

### 3.2 时序图

```text
External RL Repo                          Reward Harness
    |                                        |
    |  1) GET /healthz                       |
    |--------------------------------------->|
...
参考实现见 `reward_harness/rl/verl/reward_client.py` 中的 `RMScoringClient.healthz_check()`。
...
**路径**: `python -m reward_harness.rl.verl.prepare_training_run`
...
**路径**: `python -m reward_harness.rl.verl.finalize_training_run`
...
**路径**: `python -m reward_harness.rl.verl.run_verl_training`
...
**路径**: `reward_harness.rl.verl.reward_client.RMScoringClient`
...
```python
from reward_harness.rl.verl.reward_client import RMScoringClient

...
from reward_harness.config import LLMBackendConfig, RuntimeConfig, SearchAlgorithmConfig
from reward_harness.data_models import PromptExample, ResponseCandidate
from reward_harness.factory import ComponentFactory
from reward_harness.harness import (
...
from reward_harness.harness.session import _config_to_dict
from reward_harness.types import BackendType, EvolutionIterationScope
...
class TestSearchSessionStepExecution(unittest.TestCase):
    ...
    def test_prompt_local_resume_continues_from_checkpoint(self) -> None:
        ...
        self.assertEqual(
            checkpoint.algorithm_state.get("iteration_scope"), "prompt_local"
        )
    ...
    def test_checkpoint_interval_seconds_triggers_checkpoint_without_every_generation(
        self,
    ) -> None:
    ...
    def test_run_to_completion_uses_step_path_when_only_interval_checkpoint_enabled(
        self,
    ) -> None:
    ...
    def test_checkpoint_save_failure_is_not_silently_swallowed(self) -> None:
        ...
        class FailingStateManager(StateManager):
            def save_checkpoint(
                self, checkpoint: SearchCheckpoint, *, atomic: bool = True
            ) -> Path:
    ...
    def test_checkpoint_save_error_is_public_harness_contract(self) -> None:
        import reward_harness.harness as harness
...
class TestSearchSessionResume(unittest.TestCase):
    ...
    def test_resume_restores_rng_state(self) -> None:
        ...
        self.assertEqual(
            resumed_rng_state, _to_tuple(checkpoint.rng_state["random_state"])
        )

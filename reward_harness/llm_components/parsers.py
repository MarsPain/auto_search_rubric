...
logger = logging.getLogger("reward_harness.llm_components")
...
def payload_to_rubric(
    ...
    *,
    ...
    if not isinstance(rubric_payload, dict):
        logger.error(
            "Invalid rubric payload (not a dict): %s",
    ...
    if not isinstance(criteria, list) or not criteria:
        logger.error(
            "Missing criteria in rubric payload: %s",
            json.dumps(payload, ensure_ascii=False, indent=2),
        )
    ...
    if not normalized["criteria"]:
        logger.error(
            "All criteria were invalid after normalization: %s",
            json.dumps(payload, ensure_ascii=False, indent=2),
        )
...
def normalize_one_criterion(entry: Any, index: int) -> dict[str, Any] | None:
    ...
    if not criterion_id:
        ...
        logger.warning(
            "Criterion missing criterion_id; using fallback id=%s", criterion_id
        )
    ...
    if not text:
        ...
        logger.warning(
            "Criterion %s missing text; synthesized text=%r", criterion_id, text
        )
    criterion["text"] = text
    return criterion

    ...
    def initialize(self, item: PromptExample, *, rng: random.Random) -> Rubric:
        ...
        return Rubric(
            ...
            grading_protocol=GradingProtocol(
                num_votes=3, allow_na=True, vote_method="majority"
            ),
...
class TemplateProposer:
    ...
    def _decompose(self, rubric: Rubric, rng: random.Random) -> None:
        if not rubric.criteria:
            return
        target_idx = max(
            range(len(rubric.criteria)), key=lambda idx: len(rubric.criteria[idx].text)
        )
    ...
    def _weight_perturb(self, rubric: Rubric, rng: random.Random) -> None:
        for criterion in rubric.criteria:
            criterion.weight = max(
                0.01, criterion.weight * (1 + rng.uniform(-0.15, 0.2))
            )
...
def _judge_score(candidate: ResponseCandidate) -> float:
    ...
    source_bonus = {"strong": 0.08, "diverse": 0.05, "base": 0.0}.get(

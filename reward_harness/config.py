...
class ObjectiveConfig:
    ...
    pair_confidence_prior: float = (
        8.0  # Shrink pair-based accuracies toward 0.5 for low sample counts
    )
    ...
    def __post_init__(self) -> None:
        ...
        if (
            self.pair_budget_small < 0
            or self.pair_budget_medium < 0
            or self.pair_budget_full < 0
        ):
...
class InitializerStrategyConfig:
    ...
    strategy: InitializerStrategy = field(
        default_factory=lambda: InitializerStrategy.BACKEND
    )
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.strategy, str):
            object.__setattr__(
                self, "strategy", InitializerStrategy.from_string(self.strategy)
            )
...
class ContentExtractionConfig:
    ...
    strategy: ExtractionStrategy = field(
        default_factory=lambda: ExtractionStrategy.IDENTITY
    )
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.strategy, str):
            object.__setattr__(
                self, "strategy", ExtractionStrategy.from_string(self.strategy)
            )
...
class RuntimeConfig:
    ...
    initializer: InitializerStrategyConfig = field(
        default_factory=InitializerStrategyConfig
    )

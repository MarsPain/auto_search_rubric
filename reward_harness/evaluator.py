...
class RubricEvaluator:
        ...
        *,
        ...
        for criterion in rubric.criteria:
            ...
            if not votes_for_criterion:
                ...
                continue
            majority[criterion.criterion_id] = float(
                sum(votes_for_criterion) / len(votes_for_criterion)
            )
    ...
    def _vote_seed(
        self, prompt_id: str, candidate_id: str, rubric: Rubric, vote_idx: int
    ) -> int:
...
def compute_objective(
    ...
    *,
    ...
    sampled_pairs = _apply_pair_budget(
        pairs, item.prompt_id, rubric.rubric_id, resolved_budget
    )
    ...
    tail_acc = _shrink_toward_neutral(
        tail_acc_raw, stats.valid, config.pair_confidence_prior
    )
...
def _evaluate_pair_alignment(
    *,
    ...
    candidate_lookup = {
        candidate.candidate_id: candidate for candidate in item.candidates
    }

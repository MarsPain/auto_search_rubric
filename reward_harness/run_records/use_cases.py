...
from reward_harness.config import RuntimeConfig
from reward_harness.types import LLMRole
    ...
    while idx < len(raw_cli_args):
        ...
        for path_arg in _PATH_ARGS:
            ...
            if token.startswith(prefix):
                ...
                normalized.append(
                    f"{path_arg}={_to_absolute_path(path_token, repo_root)}"
                )
...
def build_run_manifest(
    *,
    ...
    resolved_dataset_path = (
        ...
        else (resolved_repo_root / dataset_path).resolve()
    )
    ...
    llm_snapshot = {
        ...
        "retry_backoff_base": config.llm.retry_backoff_base,
        "retry_backoff_max": config.llm.retry_backoff_max,
        "retry_jitter": config.llm.retry_jitter,
        "fail_soft": config.llm.fail_soft,

...
class MutationHistory:
    ...
    mode_history: dict[MutationMode, deque[tuple[bool, float]]] = field(
        default_factory=lambda: {mode: deque(maxlen=100) for mode in MutationMode}
    )
    generation_count: int = 0

    def record(
        self, mode: MutationMode, was_successful: bool, improvement: float
    ) -> None:
...
class AdaptiveMutationSelector:
    ...
    def __init__(
        ...
        self._weights: dict[MutationMode, float] = {mode: 1.0 for mode in MutationMode}
    ...
    def _select_exploration_decay(self) -> MutationMode:
        ...
        if is_exploration:
            ...
            weights = {
                mode: 2.0
                if mode in exploratory_modes
                else 0.5
                if mode in exploitative_modes
                else 1.0
                for mode in MutationMode
            }
        else:
    ...
    def get_state(self) -> dict[str, object]:
        ...
        return {
            ...
            "weights": {mode.value: weight for mode, weight in self._weights.items()},
        }
    ...
    def compute(
        ...
        *,
        ...
        while len(distances) < num_samples and attempts < max_attempts:
            ...
            matches = sum(1 for a, b in zip(fp1, fp2, strict=False) if a == b)

...
def _compute_diversity_score(
    ...
    for other in selected:
        ...
        matches = sum(1 for a, b in zip(rubric_fp, other_fp, strict=False) if a == b)
...
def select_parents_top_k_diverse(
    ...
    while len(selected) < num_parents and remaining:
        ...
        for idx, (rubric, breakdown) in enumerate(remaining):
            ...
            # Combined score
            combined = (
                1 - config.diversity_weight
            ) * quality + config.diversity_weight * diversity
...
def select_parents(
    ...
    return SELECTION_STRATEGIES[resolved](scored_population, num_parents, rng, config)

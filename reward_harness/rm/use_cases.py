...
def _build_runtime_snapshot(run_manifest: Mapping[str, Any] | None) -> dict[str, Any]:
    ...
    if isinstance(config_snapshot, Mapping):
        ...
        if isinstance(candidate_extraction, Mapping):
            candidate_extraction_snapshot = {
                ...
                "join_separator": str(
                    candidate_extraction.get("join_separator", "\n\n")
                ),
            }
    ...
    if isinstance(llm_snapshot, Mapping):
        default_model = str(
            llm_snapshot.get("default_model", llm_runtime_snapshot["default_model"])
        )
        llm_runtime_snapshot = {
            "base_url": str(
                llm_snapshot.get("base_url", llm_runtime_snapshot["base_url"])
            ),
            "timeout": float(
                llm_snapshot.get("timeout", llm_runtime_snapshot["timeout"])
            ),
            "max_retries": int(
                llm_snapshot.get("max_retries", llm_runtime_snapshot["max_retries"])
            ),
            "retry_backoff_base": float(
                llm_snapshot.get(
                    "retry_backoff_base", llm_runtime_snapshot["retry_backoff_base"]
                )
            ),
            "retry_backoff_max": float(
                llm_snapshot.get(
                    "retry_backoff_max", llm_runtime_snapshot["retry_backoff_max"]
                )
            ),
            "retry_jitter": float(
                llm_snapshot.get("retry_jitter", llm_runtime_snapshot["retry_jitter"])
            ),
            "fail_soft": bool(
                llm_snapshot.get("fail_soft", llm_runtime_snapshot["fail_soft"])
            ),
            "default_model": default_model,
...
def _extract_rubric_map(search_output: Mapping[str, Any]) -> dict[str, Rubric]:
    ...
    if not isinstance(best_rubrics, list) or not best_rubrics:
        raise ArtifactValidationError(
            "best_rubrics", "search output missing best_rubrics entries"
        )
...
def validate_rm_artifact(
    ...
    *,
    ...
    if set(source_rubrics.keys()) != set(artifact.rubric.keys()):
        raise ArtifactValidationError(
            "rubric", "prompt_id set does not match source search output"
        )
    ...
    *,
    ...
    for manifest_path in sorted(out_dir.glob("*.json")):
        ...
        if manifest.deployment_target != deployment_target:
            continue
        if (
            latest_for_target is None
            or manifest.deployed_at_utc > latest_for_target.deployed_at_utc
        ):
...
def record_deploy_manifest(
    *,
    ...
    resolved_rollback_policy = dict(
        rollback_policy or {"strategy": "rollback_to_previous_artifact"}
    )

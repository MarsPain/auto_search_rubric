...
def _validate_runtime_snapshot(snapshot: Mapping[str, Any]) -> None:
    ...
    if not isinstance(extraction, Mapping):
        raise ArtifactValidationError(
            "runtime_snapshot.extraction", "must be an object"
        )
...
class RMArtifact:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.rubric:
            raise ArtifactValidationError(
                "rubric", "must include at least one prompt rubric"
            )
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "RMArtifact":
        ...
        if not isinstance(rubric_raw, Mapping):
            raise ArtifactValidationError(
                "rubric", "must be a mapping from prompt_id to rubric"
            )
...
class DeployManifest:
    ...
    def __post_init__(self) -> None:
        if not self.deploy_id.strip():
            raise ArtifactValidationError(
                "deploy_id", "must not be empty", schema="DeployManifest"
            )
        ...
        if not self.artifact_id.strip():
            raise ArtifactValidationError(
                "artifact_id", "must not be empty", schema="DeployManifest"
            )
        if not self.artifact_path.strip():
            raise ArtifactValidationError(
                "artifact_path", "must not be empty", schema="DeployManifest"
            )
        if not self.deployed_by.strip():
            raise ArtifactValidationError(
                "deployed_by", "must not be empty", schema="DeployManifest"
            )
        ...
        if not self.dataset_hash.strip():
            raise ArtifactValidationError(
                "dataset_hash", "must not be empty", schema="DeployManifest"
            )
        if not self.config_hash.strip():
            raise ArtifactValidationError(
                "config_hash", "must not be empty", schema="DeployManifest"
            )
        if self.schema_version != "1.0":
            raise ArtifactValidationError(
                "schema_version",
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "DeployManifest":
        ...
        previous_artifact_id = (
            None if previous_artifact_id_raw is None else str(previous_artifact_id_raw)
        )
        return cls(

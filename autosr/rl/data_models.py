...
class TrainingValidationError(Exception):
    ...
    def __init__(self, field: str, message: str, *, schema: str = "TrainingManifest") -> None:
...
class TrainingManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.dataset.get("dataset_version"):
            raise TrainingValidationError("dataset.dataset_version", "must not be empty")
...
class TrainingResultManifest:
    ...
    def __post_init__(self) -> None:
        ...
        if self.status == "succeeded":
            ...
            if not any(
                self.output.get(k) for k in ("checkpoint_path", "model_artifact_path", "log_path")
            ):
                raise TrainingValidationError(
                    "output",
        ...
        if self.status == "failed":
            if not self.failure:
                raise TrainingValidationError(
                    "failure", "required when status is failed", schema="TrainingResultManifest"
                )
            if not self.failure.get("type"):
                raise TrainingValidationError(
                    "failure.type", "required when status is failed", schema="TrainingResultManifest"
                )
    ...
    def from_json(cls, payload: str) -> "TrainingResultManifest":
        try:
            ...
            raise TrainingValidationError("json", f"invalid JSON: {exc}", schema="TrainingResultManifest") from exc
        return cls.from_dict(data)
...
class EvalReport:
    ...
    def __post_init__(self) -> None:
        if not self.eval_run_id.strip():
            raise TrainingValidationError("eval_run_id", "must not be empty", schema="EvalReport")
        if not self.training_run_id.strip():
            raise TrainingValidationError(
                "training_run_id", "must not be empty", schema="EvalReport"
            )
        if not isinstance(self.benchmark, dict):
            raise TrainingValidationError("benchmark", "must be an object", schema="EvalReport")
        if not self.benchmark.get("name"):
            raise TrainingValidationError("benchmark.name", "must not be empty", schema="EvalReport")
        ...
        if not isinstance(self.metrics, dict) or not self.metrics:
            raise TrainingValidationError("metrics", "must be a non-empty object", schema="EvalReport")
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "EvalReport":
        ...
        return cls(
            ...
            training_run_id=str(data.get("training_run_id", "")),
            evaluated_at_utc=str(data.get("evaluated_at_utc", datetime.now(timezone.utc).isoformat())),
    ...
    def from_json(cls, payload: str) -> "EvalReport":
        try:
            ...
            raise TrainingValidationError("json", f"invalid JSON: {exc}", schema="EvalReport") from exc
        return cls.from_dict(data)
...
class LineageIndex:
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "LineageIndex":
        return cls(
            ...
            updated_at_utc=str(data.get("updated_at_utc", datetime.now(timezone.utc).isoformat())),
        )

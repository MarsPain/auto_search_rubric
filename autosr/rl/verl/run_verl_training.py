...
from autosr.rl.data_models import TrainingManifest
from autosr.rl.registry import DuplicateEntryError, ExperimentRegistry, RegistryError
from autosr.rl.verl.prepare_training_run import generate_training_run_id
...
def _run_finalize(
    ...
    *,
    ...
    if status == "succeeded":
        ...
        cmd += [
            "--failure-type", failure_type or "UnknownError",
            "--failure-message", failure_message or "Training process exited with non-zero code",
            "--failure-stage", failure_stage or "training",
        ]
...
def main() -> None:
    ...
    exit_code, started_at, finished_at, duration, trainer_failure_type, trainer_failure_message = _run_trainer(
        args,
        training_run_id,
    )
    if exit_code == 0:
    ...
    rc = _run_finalize(
        ...
        failure_message=trainer_failure_message or (f"Trainer exited with code {exit_code}" if exit_code != 0 else ""),
